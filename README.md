...
База H2 будет храниться в локальном каталоге `./data`, который пробрасывается в контейнер как `/app/data`. Смена владельца обязательна: контейнер запускается от пользователя `app` с UID/GID 1000 и без неё не сможет создать файлы базы данных.
...
### AccessDeniedException на `/app/data/batrobot-db.mv.db`

Контейнер не может создать файлы базы данных — директория `data` на хосте недоступна для записи пользователю `app` (UID 1000).

Быстрое исправление без пересборки образа:
